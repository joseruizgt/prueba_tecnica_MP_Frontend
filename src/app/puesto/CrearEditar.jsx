...
export const CrearEditar = ({ flagCrear }) => {
    ...
    const {individualJob, listJobH} = usePuesto()
    const { id } = useParams();

    //ESTADOS
    const [text, setText] = useState('');
    const [open, setOpen] = useState(false);
    const [dataPuesto, setDataPuesto] = useState(null);
    let watchItems = watch();

    const crearEditarPuesto = (data) => {
        setDataPuesto(data);
        setOpen(true);
    }
    ...
    useEffect(() => {
        if (!flagCrear) {
            listJobH(undefined, undefined, 0, decryptItem(id));
        }
    ...
    useEffect(() => {
        if (!flagCrear) {
            setValue('puesto', individualJob.puesto);
            setValue('descripcion', individualJob.descripcion);
        }
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [individualJob])
    ...
    return (
        <ThemeProvider theme={theme}>
            <div className="flex">
                <div className="w-2/12" >
                    <h3 className='font-extrabold text-md text-blue-900 mb-2'>
                        {GESTION_PUESTOS.actuales}
                    </h3>

                    {
                        !flagCrear && Object.keys(individualJob).length > 0
                            // && list.length > 0
                            ?
                            <ul style={{ listStyleType: 'disc', color: 'gray', fontSize: 15 }}>
                                <li>{`Puesto: ${individualJob.puesto}`}</li>
                                <li>{`Descripción: ${individualJob.descripcion}`}</li>
                            </ul>
                            :
                            <ul style={{ listStyleType: 'disc', color: 'gray', fontSize: 15 }}>
                                <li>Sin registros</li>
                ...
                <form className="w-10/12" onSubmit={handleSubmit((data) => crearEditarPuesto(data))}>
                    <div>
                        <div className="ml-5 mb-5 flex justify-between">
                            <div>
                                <h2 className='font-extrabold text-3xl text-blue-900'>
                                    {
                                        flagCrear ?
                                        GESTION_PUESTOS.creacion
                                        :
                                        GESTION_PUESTOS.edicion
                                    }
                                </h2>
                                <h3 className=' text-gray-500 mt-5'>
                                    {GESTION_PUESTOS.completar}
                                </h3>
                            </div>
                            <div className="my-2">
                                ...
                                <Button onClick={() => navigate('/administracion/puesto')} style={{ color: '#1e3a8a', fontWeight: 600 }} variant="outlined">Cancelar</Button>
                            </div>
                        </div>
                        <Divider />
            ...
            <ModalComponent open={open} setOpen={setOpen}>
                <GuardarModal dataPuesto={dataPuesto} setOpen={setOpen} flagCrear={flagCrear} />
            </ModalComponent>
        </ThemeProvider>
    )
}
