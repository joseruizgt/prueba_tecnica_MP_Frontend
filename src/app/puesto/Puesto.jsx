...
import { GESTION_PUESTOS } from '../../@utilities/constants';
...
export const Puesto = () => {
  ...
  const {listJobH, listJob, total} = usePuesto();
  ...
  const columns = [
    { id: 'puesto', label: 'Puesto', minWidth: 170, align: 'center' },
  ...
  const optionsMenuItems = [
    {
      icono: <EditIcon />,
      accion: GESTION_PUESTOS.menuItem2
    }
  ];
  //ACCIONES DE LA TABLA PARA USUARIOS ACTIVOS Y DESHABILITADOS
  const acciones = (idPuesto) => {
    return (
      <ThemeProvider theme={theme}>
        <div className='flex justify-end'>
          <Button onClick={() => modalVisualizar(idPuesto)} variant="outlined">{GESTION_PUESTOS.accion1}</Button>
          <MenuItems id={idPuesto} selectOption={setOptionsMenu} options={optionsMenuItems} />
        </div>
      </ThemeProvider>
    )
  }
  ...
  const modalVisualizar = (id) => {
    setPuesto(listJob.find(item => item.idPuesto === id));
    setOpen(true);
  }

  const lista = (search) => {
    listJobH(search, rowsPerPage, page);
  }
  ...
  useEffect(() => {
    if (!optionsMenu.ejecucion) {
        setPuesto(optionsMenu.idItem ? listJob.find(item => item.idPuesto === optionsMenu.idItem) : {});
      if (optionsMenu.option === GESTION_PUESTOS.menuItem2) {
        navigate(`/administracion/puesto/editar/${encriptItem(optionsMenu.idItem)}`)
      }
    } else {
      setPage(1);
      setValue('busqueda', '');
    ...
    <>
      ...
      <ThemeProvider theme={theme}>
        <div className='my-8 w-full'>
          ...
          <div>
            <InputSearch2 register={register} name='busqueda' etiqueta='Buscar puesto' textoBusqueda='Por nombre de puesto' />
          </div>
        </div>
      </ThemeProvider>
      ...
      <Box sx={{ '& > :not(style)': { m: 1 } }}>
        <Fab color="success" aria-label="add" sx={{ position: 'fixed', bottom: '20px', right: '20px' }} onClick={() => navigate('/administracion/puesto/crear')}>
          <AddIcon />
        </Fab>
      </Box>

      <ModalComponent open={open} setOpen={setOpen}>
        <Detalles setOpen={setOpen} puesto={puesto} />
      </ModalComponent>
    </>
  );
}