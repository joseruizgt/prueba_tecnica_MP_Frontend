...
export const GuardarModal = ({ dataPuesto, setOpen, flagCrear }) => {
    ...
    const {createJobH, updateJobH} = usePuesto()
    const navigate = useNavigate();
    const { id } = useParams();

    const crearEditar = () => {
        if (flagCrear) {
            createJobH(dataPuesto, navigate);
            setOpen(false);
        } else {
            updateJobH(dataPuesto, decryptItem(id));
            navigate('/administracion/puesto');
        }
    }
    ...
    return (
        <ThemeProvider theme={theme}>
            <Typography id="modal-modal-title" variant="h6" component="h2" style={{ color: '#1e3a8a', fontWeight: 600, }}>
                {`${flagCrear ? 'Guardar puesto' : 'Guardar cambios'}`}
            </Typography>
